...
export function Chat({ scope, attribute, loading}) {
    ...
    return (
        <div className="w-200 h-full pb-1/19 pt-1/19 absolute justify-center items-center flex flex-col">
        {
            
            <h2 className="align-left"> Messages:</h2> 
                           ...
                           round={round} player={player}/>
        {
            ...
            stage.get("name") == 'send' ?
             <InputBox onNewMessage={handleNewMessage} buttonStyles='w-9 h-9 p-2 text-sm'/> : player.stage && player.stage.get("submit") ? <div className="text-gray-600 text-sm text-center mb-2"> Thank you! The next stage will start when all the other 
