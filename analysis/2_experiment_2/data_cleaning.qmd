...
decode1 <- read_json(here('analysis/2_simulations/games_dontoverwrite.json'), simplifyVector = TRUE) %>%
  as_tibble() %>%
...
decode3 <- read_json(here('analysis/2_simulations/games_2_dontoverwrite.json'), simplifyVector = TRUE) %>%
  as_tibble() %>%
  rename(nCrittersGame = totalCritters) %>%
  rename(designatedGame = gameID)%>%
...
```{r}
decode_combined_with_direction <-merge(decode_combined, low_direction[, c("treatment", "representation_bin", "direction")], by = c("treatment", "representation_bin"), all.x = TRUE)%>% 
distinct()

# find rows from decode_combined that were not matched
unmatched_rows <- anti_join(low_direction, decode_combined, by = c("treatment", "representation_bin"))
...
```{r}
write_json(decode_combined_with_direction, "missing_games_jan9_2.json", pretty = TRUE)
```
