...
d.all_rebinned <- d.all %>%
  mutate(
 x_bin = cut(abs(entropy), breaks = seq(0, 2, .25), right = TRUE, include.lowest = TRUE, dig.lab = 2),
      y_bin = cut(abs(representation), breaks = seq(0, 1, .12), right = TRUE, include.lowest = TRUE, dig.lab = 2),
 nCrittersGame_bin = cut(nCrittersGame, breaks = seq(15, 100, by = 9), right = TRUE, include.lowest = TRUE)
    )


# data preparation
heatmap_test_rebinned <- d.all_rebinned %>%
  
  group_by(representation_bin, entropy_bin, nCrittersGame_bin, treatment) %>%
  summarize(errorAvg = mean(error, na.rm = TRUE), .groups = "drop")

ggplot(heatmap_test_rebinned, aes(x = representation_bin, y = entropy_bin, color = errorAvg)) +
  geom_tile(aes(fill = errorAvg), color = "white")+
  scale_color_gradient(low = "lightgreen", high = "darkblue") +
  facet_wrap(~nCrittersGame_bin, labeller = label_both) +  # Facet by nCrittersGame_bin
  theme_few() +
  labs(title = "2D Slices of 3D Cube Heatmap",
