...
```{r}
d.all <- d.all %>% mutate(
  entropy_minmax = ifelse(entropy <= median(entropy, na.rm = TRUE), "low", "high"),
  representation_minmax = ifelse(representation <= median(representation, na.rm = TRUE), "low", "high"),
  nCrittersGame_minmax = ifelse(nCrittersGame <= median(nCrittersGame, na.rm = TRUE), "low", "high"),
  group = paste(entropy_minmax, representation_minmax, nCrittersGame_minmax, sep = "/")) %>%
  group_by(representation_bin, entropy_bin, nCrittersGame_bin, treatment) %>%
  mutate(errorAvg = mean(error, na.rm = TRUE)) %>% ungroup() # %>% filter(idx==8)

lm_model <- lm(errorAvg ~ group, data = d.all)

counts <- d.all %>%
  group_by(group) %>%
  summarise(nGames = n())
# scatterplot
ggplot(d.all, aes(x = group, y = errorAvg)) +
  geom_boxplot(fill = "skyblue", alpha = 0.6) +
  stat_compare_means(aes(group = group), method = "anova", label = "p.signif") +  # or method = "kruskal.test"
  labs(x = "Group", y = "Error Average") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

lm_model <- lm(errorAvg ~ group, data = d.all)

# anova to test significance of the group variable
anova(lm_model)
