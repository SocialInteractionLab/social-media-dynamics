...
ggplot(subset(d.allmsg_exp1_2, round == 12), aes(x = s_count, y = error)) +
...
ggplot(subset(d.allmsg2, round == 8), aes(x = s_count, y = error)) +
  geom_point() + geom_smooth(method="lm")+
  labs(
    y = "error",
    x = "s",
...
```{r}
combined_df3 <- bind_rows(d.allmsg2[d.allmsg2$round == 8, ], 
                          d.allmsg_exp1_2[d.allmsg_exp1_2$round == 12, ])

heatmap_test <- combined_df3%>%
