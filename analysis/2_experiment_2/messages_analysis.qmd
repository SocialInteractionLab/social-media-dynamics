...
```{r}
summary_data_long <- summary_data_long %>%                    # make sb the reference
  mutate(message_type = fct_relevel(message_type, 'sb_count',
                                    'pn_count', 'pb_count', 'sn_count'))

m <- lmer(factor(decile) ~ message_type + (1 | gameID),            # mixed model
          data = summary_data_long)

emmeans(m, pairwise ~ message_type, adjust = "tukey") 
```
