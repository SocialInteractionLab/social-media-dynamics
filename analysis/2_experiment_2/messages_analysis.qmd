...
summary_data_long <- summary_data %>%
  select(gameID, avg_error, pn_prop, sn_prop, pb_prop, sb_prop) %>%
...
# Normalize proportions to ensure they sum to 1 within each decile (across all gameIDs)
...
ggplot(summary_data_long, aes(x = factor(decile), y = proportion, fill = message_type)) +
  ...
  scale_fill_brewer(palette = 'Accent', 
                    labels = c("pn_prop" = "personal number", "sn_prop" = "social number",
                               "pb_prop" = "personal belief", "sb_prop" = "social belief")) +
  labs(title = "",
       x = "error (binned)",
       y = "proportion",
       fill = "message type") +
