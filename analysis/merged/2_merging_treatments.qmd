...
d.treatments.b1 <- read_csv(here('data/pilot/exp2/batch1/games.csv')) %>%
  select(gameID = id, treatmentName) %>% 
  distinct()
d.treatments.b2 <- read_csv(here('data/pilot/exp2/batch2/games.csv')) %>%
  select(gameID = id, treatmentName) %>% 
  distinct()
d.treatments.b3 <- read_csv(here('data/pilot/exp2/batch3/games.csv')) %>%
  select(gameID = id, treatmentName) %>% 
  distinct()
d.treatments.b4 <- read_csv(here('data/pilot/exp2/batch4/games.csv')) %>%
  select(gameID = id, treatmentName) %>% 
  distinct()
# map game IDs to their treatments
d.treatmentse1 <- read_csv(here('data/pilot/exp1/games.csv')) %>%
  select(gameID = id, treatmentName) %>% 
  distinct()
...
# map game IDs to their treatments
d.players.b1 <- read_csv(here('data/pilot/exp2/batch1/players.csv')) %>%
  mutate(nSquirrels = str_count(emojiArray, '🐿️'),
         nRabbits = str_count(emojiArray, '🐇')) %>%
  select(playerID = id, gameID, nSquirrels, nRabbits) %>%
  distinct()

d.players.b2 <- read_csv(here('data/pilot/exp2/batch2/players.csv')) %>%
  mutate(nSquirrels = str_count(emojiArray, '🐿️'),
         nRabbits = str_count(emojiArray, '🐇')) %>%
  select(playerID = id, gameID, nSquirrels, nRabbits) %>%
  distinct()

d.players.b3 <- read_csv(here('data/pilot/exp2/batch3/players.csv')) %>%
  mutate(nSquirrels = str_count(emojiArray, '🐿️'),
         nRabbits = str_count(emojiArray, '🐇')) %>%
  select(playerID = id, gameID, nSquirrels, nRabbits) %>%
  distinct()

d.players.b4 <- read_csv(here('data/pilot/exp2/batch4/players.csv')) %>%
  mutate(nSquirrels = str_count(emojiArray, '🐿️'),
         nRabbits = str_count(emojiArray, '🐇')) %>%
  select(playerID = id, gameID, nSquirrels, nRabbits) %>%
  distinct()
...
# map game IDs to their treatments
d.playerse1 <- read_csv(here('data/pilot/exp1/players.csv')) %>%
  mutate(nSquirrels = str_count(emojiArray, '🐿️'),
         nRabbits = str_count(emojiArray, '🐇')) %>%
  select(playerID = id, gameID, nSquirrels, nRabbits) %>%
  distinct()
...
d.rounds.b1 <- read_csv(here('data/pilot/exp2/batch1/rounds.csv')) %>%
  select(gameID, roundID = id, idx) %>%
  distinct()
d.rounds.b2 <- read_csv(here('data/pilot/exp2/batch2/rounds.csv')) %>%
  select(gameID, roundID = id, idx) %>%
  distinct()
d.rounds.b3 <- read_csv(here('data/pilot/exp2/batch3/rounds.csv')) %>%
  select(gameID, roundID = id, idx) %>%
  distinct()
d.rounds.b4 <- read_csv(here('data/pilot/exp2/batch4/rounds.csv')) %>%
  select(gameID, roundID = id, idx) %>%
  distinct()
# map round IDs to their indices
d.roundse1 <- read_csv(here('data/pilot/exp1/rounds.csv')) %>%
  select(gameID, roundID = id, idx) %>%
  distinct()
...
d.msgs.b1 <- read_csv(here('data/pilot/exp2/batch1/games.csv')) %>% 
...
d.msgs.b2 <- read_csv(here('data/pilot/exp2/batch2/games.csv')) %>% 
  mutate(messages = ifelse(is.na(messages), '[{}]', messages)) %>%
  select(gameID = id, treatmentName, messages) %>%
  mutate(messages = map(messages, .f = ParseJSONColumn)) %>%
  unnest(messages) %>% unnest(messages)%>%
...
d.msgs.b3 <- read_csv(here('data/pilot/exp2/batch3/games.csv')) %>% 
  mutate(messages = ifelse(is.na(messages), '[{}]', messages)) %>%
  select(gameID = id, treatmentName, messages) %>%
  mutate(messages = map(messages, .f = ParseJSONColumn)) %>%
  unnest(messages) %>% unnest(messages)%>%
...
d.msgs.b4 <- read_csv(here('data/pilot/exp2/batch4/games.csv')) %>% 
  mutate(messages = ifelse(is.na(messages), '[{}]', messages)) %>%
  select(gameID = id, treatmentName, messages) %>%
  mutate(messages = map(messages, .f = ParseJSONColumn)) %>%
  unnest(messages) %>% unnest(messages)%>%
...
# create long-form dataframe of all messages sent in each game
d.msgse1 <- read_csv(here('data/pilot/exp1/games.csv')) %>% 
  mutate(messages = ifelse(is.na(messages), '[{}]', messages)) %>%
  select(gameID = id, treatmentName, messages) %>%
...
d.guesses.b1 <- read_csv(here('data/pilot/exp2/batch1/playerStages.csv')) %>% 
  select(gameID, roundID, playerID, guess) %>%
  left_join(d.rounds.b1) %>%
  left_join(d.treatments.b1) %>%
  left_join(d.players.b1) 

d.guesses.b2 <- read_csv(here('data/pilot/exp2/batch2/playerStages.csv')) %>% 
  select(gameID, roundID, playerID, guess) %>%
  left_join(d.rounds.b2) %>%
  left_join(d.treatments.b2) %>%
  left_join(d.players.b2) 

d.guesses.b3 <- read_csv(here('data/pilot/exp2/batch3/playerStages.csv')) %>% 
  select(gameID, roundID, playerID, guess) %>%
...
d.guesses.b4 <- read_csv(here('data/pilot/exp2/batch4/playerStages.csv')) %>% 
  select(gameID, roundID, playerID, guess) %>%
...
# create long-form dataframe of all guesses made in each game
d.guessese1 <- read_csv(here('data/pilot/exp1/playerStages.csv')) %>% 
