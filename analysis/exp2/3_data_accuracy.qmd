...
ggplot(d.all_plt, aes(x = nCritters, y = error, color = treatment, group = treatment)) +
  ...
  scale_color_brewer(palette = 'Accent') +
  theme_few() +
  labs(x = "critters", y = "error", color = "") +
  theme(
    legend.position = 'top',
    axis.text.x = element_text(angle = 15, hjust = 1), aspect.ratio=1
  )
...
```{r}
ggplot(d.all_plt, aes(representation, error, colour=treatment, group=treatment)) +
  geom_smooth(method='lm', se=FALSE) +
  scale_colour_brewer(palette='Accent') +
  theme_few() +
  labs(x='representation', y='error', colour='') +
  theme(
    legend.position='top',
    axis.text.x=element_text(angle=15, hjust=1),
    aspect.ratio=1      # height = 0.5 × width
  )
