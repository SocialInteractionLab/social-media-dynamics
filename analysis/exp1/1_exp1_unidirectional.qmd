...
d.treatments <- read_csv(here('data/pilot/exp1/games.csv')) %>%
  select(gameID = id, treatmentName) %>% 
  distinct()

# map game IDs to their treatments
d.players <- read_csv(here('data/pilot/exp1/players.csv')) %>%
...
d.rounds <- read_csv(here('data/pilot/exp1/rounds.csv')) %>%
  select(gameID, roundID = id, idx) %>%
  distinct()

# create long-form dataframe of all messages sent in each game
d.msgs <- read_csv(here('data/pilot/exp1/games.csv')) %>% 
...
d.guesses <- read_csv(here('data/pilot/exp1/playerStages.csv')) %>% 
